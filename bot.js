...
function messageChecker(oldMessage, newMessage) {
    ...
    if (panicMode[message.guild.id]) {
        if (msg == "wod:panic" && (message.member.roles.find("name", "Admin")  || message.member.roles.find("name", "Upper Council of Explorers"))) {
    ...
    if (message.author.id != 280495817901473793 && !message.author.bot) {
        ...
        if (doModeration[message.guild.id]) { //Check if we should do moderation on this server
            ...
            if (message.author.id != 282048599574052864 && msg.search(/\b(kys|kill yourself)\b/i) != -1) {
                ...
                if (message.guild.id == 277922581312) { //AstralPhaser
                    client.channels.get("28251318004747").sendMessage(getBoshyTime(message.guild) + " PING! <@" + auth.id + "> wrote \"kys\" on " + message.channel.name + ".");
                } else if (message.guild.id == 23449330349056) { //ShiftOS
                    client.channels.get("28251257658880").sendMessage(getBoshyTime(message.guild) + " PING! <@" + auth.id + "> wrote \"kys\" on " + message.channel.name + ".");
        ...
        if (msg.toLowerCase().startsWith("wod:") || msg.toLowerCase().startsWith("mot:")) {
        ...
        if (msg.toLowerCase().startsWith("wod:") && !commandProcessed) {
            //Check for moderator/admin permission
            
            //Moderator ID: 282068037664768001
            //Admin ID:     282068065619804160
            if (message.member.roles.find("name", "Admin") || message.member.roles.find("name", "Moderator") || message.member.roles.find("name", "Victor Tran King") || message.member.roles.find("name", "Mod") || message.member.roles.find("name", "Upper Council of Explorers") || message.member.roles.find("name", "Lower Council of Explorers")) { //Thanks Aren! :D
                var command = msg.substr(4);
                switch (command) {
                    case "filter":
                        if (message.guild.id != 277922530973581312) {
                    ...
                    case "interrogate":
                        if (message.guild.id != 277922530973581312 && message.guild.id != 234414439330349056) {
                            message.author.send(':no_entry_sign: ERROR: Unable to use that command in this server.');
                        } else {
                            if (interrogMember == null) {
                                message.author.send(':no_entry_sign: ERROR: No user to interrogate. See mod:help for more information.');
                            } else {
                                if (message.guild.id == 277922530973581312) {
                    ...
                    case "reboot":
                        ...
                        break;
                    case "jail":
                        if (message.guild.id != 310863118831976458) {
                            message.author.send(':no_entry_sign: ERROR: Unable to use that command in this server.');
                        } else {
                            if (jailMember == null) {
                                message.author.send(':no_entry_sign: ERROR: No user to jail. See mod:help for more information.');
                            } else {
                                jailMember.addRole(jailMember.guild.roles.get("311202966159753216"));
...
client.on('guildMemberAdd', function(guildMember) {
    if (guildMember.guild.id == 234439330349056 || guildMember.guild.id == 2779973581312) {
        var channel;
        if (guildMember.guild.id == 277922973581312) {
            channel = client.channels.get("28485830695936");
            console.log(guildMember.displayName + " joined AstralPhaser Central");
            interrogMember = guildMember;
        } else {
            channel = client.channels.get("284826813467136");
...
client.on('guildMemberUpdate', function(oldUser, newUser) {
    if (newUser.guild.id == 277922530971312) {
        if (/*!oldUser.roles.find("name", "I Broke The Rules!") &&*/ newUser.roles.find("name", "Moderator")) {
            console.log("Someone broke the rules!");
            client.channels.get("2779432040").sendMessage("<@" + newUser.id + "> :oncoming_police_car: You are now in jail. Appeal here to get out of jail. If you do not appeal successfully within 24 hours, an admin will **ban** you from the server.\n\n" + 
        ...
        if (/*!oldUser.roles.find("name", "I Broke The Rules!") &&*/ newUser.roles.find("name", "Moderator")) {
            ...
            client.channels.get("2926340311808").sendMessage("<@" + newUser.id + "> :oncoming_police_car: You are in interrogation because due to recent server attacks, we want to make sure you're not someone else that has already been banned. A mod or an admin will come and speak to you shortly. Thanks! :)");
        }
        
        if (newUser.nickname != oldUser.nickname) {
            var channel = client.channels.get("28566897621697"); //Admin Bot warnings
    ...
    } else if (newUser.guild.id == 2344144339056) {
        if (/*!oldUser.roles.find("name", "I Broke The Rules!") &&*/ newUser.roles.find("name", "Moderator")) {
            console.log("Someone is in interrogation!");
            client.channels.get("295337415826").sendMessage("<@" + newUser.id + "> :oncoming_police_car: You are in interrogation because due to recent server attacks, we want to make sure you're not someone else that has already been banned. A mod or an admin will come and speak to you shortly. Thanks! :)");
        }
    }
});
...
client.on('userUpdate', function(oldUser, newUser) {
    if (newUser.guild != null) {
        if (newUser.guild.id == 27792253097312) {
            if (newUser.username != oldUser.username) {
                var channel = client.channels.get("2856689790621697"); //Admin Bot warnings
...
client.on('guildMemberRemove', function(user) {
    if (user.roles.find("name", "I Broke The Rules!")) {
        ...
        client.channels.get("277943831040").sendMessage(":arrow_left: <@" + user.id + "> has left the server in jail.");
    }
    
    if (user.guild != null) {
        if (user.guild.id == 277922581312 || user.guild.id == 2344149349056) {
            var channel;
            if (user.guild.id == 27792253731312) {
                channel = client.channels.get("28483760695936");
                console.log(user.displayName + " left AstralPhaser Central");
            } else {
                channel = client.channels.get("284826413467136");
...
client.on('messageDelete', function(message) {
    if (message.content.startsWith("mot:") || message.content.startsWith("wod:")) return; //Don't want to warn about AstralMod deleted messages
    ...
    if (message.guild != null) {
        ...
        if (message.guild.id == 277922530981312) { //AstralPhaser Central
            channel = client.channels.get("0439711258968065");
        } else if (message.guild.id == 234414439349056) { //ShiftOS
            channel = client.channels.get("290427158292480");
        } else if (message.guild.id == 278824407743463424) { //theShell
...
client.on('messageDeleteBulk', function(messages) {
    ...
    if (messages.first().guild != null) {
        ...
        if (messages.first().guild.id == 27792253581312) { //AstralPhaser Central
            channel = client.channels.get("290439758968065");
        } else if (messages.first().guild.id == 23439330349056) { //ShiftOS
            channel = client.channels.get("290442328292480");
...
client.on('messageUpdate', function(oldMessage, newMessage) {
    ...
    if (oldMessage.guild != null) {
        if (oldMessage.guild.id == 277922530581312) { //AstralPhaser Central
            channel = client.channels.get("29043258968065");
        } else if (oldMessage.guild.id == 23441443349056) { //ShiftOS
            channel = client.channels.get("290442292480");
